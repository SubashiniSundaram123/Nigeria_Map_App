...
# # --- Load Excel ---
# file_path = "data/Nigeria NewInventoryMinelist.xlsx"
# try:
...
#     search_text = st.text_input("Search by Keyword:", placeholder="Enter any search term...")

#     # Company name filter
#     company_col = "NAME OF COMPANY/MINERAL TITLE HOLDER"
#     company_options = ["All Companies"]

#     # Try to find company name column with variations
#     if company_col in valid_coords.columns:
#         companies = valid_coords[company_col].dropna().unique()
#     elif "NAME OF COMPANY/ MINERAL TITLE HOLDER" in valid_coords.columns:
#         company_col = "NAME OF COMPANY/ MINERAL TITLE HOLDER"
#         companies = valid_coords[company_col].dropna().unique()
#     elif "COMPANY NAME" in valid_coords.columns:
#         company_col = "COMPANY NAME"
#         companies = valid_coords[company_col].dropna().unique()
#     else:
#         companies = []

#     company_options.extend(sorted(companies))
#     selected_company = st.selectbox("Filter by Company:", company_options)

# with filter_col2:
#     # Mineral filter
#     mineral_col = "MINERAL" if "MINERAL" in valid_coords.columns else None
#     if mineral_col:
#         mineral_options = ["All Minerals"]
#         mineral_options.extend(sorted(valid_coords[mineral_col].dropna().unique()))
#         selected_mineral = st.selectbox("Filter by Mineral:", mineral_options)
#     else:
#         selected_mineral = "All Minerals"

#     # State/Mine Area filter
#     area_col = None
#     if "STATE" in valid_coords.columns:
#         area_col = "STATE"
#     elif "MINE AREA" in valid_coords.columns:
#         area_col = "MINE AREA"
#     elif "LOCATION" in valid_coords.columns:
#         area_col = "LOCATION"

#     if area_col:
#         area_options = ["All Areas"]
#         area_options.extend(sorted(valid_coords[area_col].dropna().unique()))
#         selected_area = st.selectbox(f"Filter by {area_col}:", area_options)
#     else:
#         selected_area = "All Areas"

# # Apply filters
# filtered_df = valid_coords.copy()
...
# # Company filter
# if selected_company != "All Companies" and company_col in filtered_df.columns:
#     filtered_df = filtered_df[filtered_df[company_col] == selected_company]

# # Mineral filter
# if selected_mineral != "All Minerals" and mineral_col and mineral_col in filtered_df.columns:
#     filtered_df = filtered_df[filtered_df[mineral_col] == selected_mineral]

# # Area filter
# if selected_area != "All Areas" and area_col and area_col in filtered_df.columns:
#     filtered_df = filtered_df[filtered_df[area_col] == selected_area]

# # Show number of results
# st.markdown(f"**{len(filtered_df)} titles found** matching your criteria")
# st.markdown("---")


# # --- Generate Excel and CSV data ---
# def get_csv_data(df):
...
#     # Fix: Correctly check for the company name column
#     # The exact column name as mentioned in the error message
#     exact_company_col = "NAME OF COMPANY/MINERAL TITLE HOLDER"

#     # First try the exact column name
#     company_name = ""
#     if exact_company_col in row:
#         company_name = row[exact_company_col]
#     # Try alternative versions with different spacing
#     elif "NAME OF COMPANY/ MINERAL TITLE HOLDER" in row:
#         company_name = row["NAME OF COMPANY/ MINERAL TITLE HOLDER"]
...
#                         row.get('MINERAL TITLE HOLDER', ''))

#     # Use company name as the popup header instead of "No Title"
#     popup_html += f"<h4 style='margin:0;padding-bottom:5px;border-bottom:1px solid #ccc;'>{company_name or 'Unknown Company'}</h4>"
...
#     details_html = ""  # Initialize details_html before using it
...
#                                     "NAME OF COMPANY", "MINERAL TITLE HOLDER",
#                                     "NAME OF COMPANY/ MINERAL TITLE HOLDER", exact_company_col,
#                                     "STATE"]
...
#     # Fix: Use the same company_name logic we used for the popup
...
# with col2:
#     # CSV button at top with completely unique key
#     st.download_button(
#         label="⬇️ Download CSV",
#         data=get_csv_data(filtered_df),
#         file_name="filtered_titles.csv",
#         mime="text/csv",
#         key="csv_download_top_section"  # Fixed: Much more unique key
#     )
# with col3:
#     # Excel button at top with completely unique key
#     st.download_button(
#         label="⬇️ Download Excel",
#         data=get_excel_data(filtered_df),
#         file_name="filtered_titles.xlsx",
#         mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
#         key="excel_download_top_section"  # Fixed: Much more unique key
#     )
